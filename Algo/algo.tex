...
\begin{document}
	...
	众所周知，字符串模式匹配是计算机科学最为古老的问题之一。朴素的字符串匹配算法可以描述为：对于待匹配字符串（下称之为文本）的每一个位置，逐个字符比较从该位置开始是否与模式串（下称之为模板）完全匹配。时间复杂度为$O(n*m)$，十分低效。于是，我斗胆用我粗浅的理解介绍一种更快、更优雅，而且基本是线性复杂度的字符串匹配算法——KMP算法(The Knuth-Morris-Pratt Algorithm)。
	...
	滑动的过程中，我们发现"oo"和"go"分别是部分匹配的后两位、前两位；"o"和"g"则分别是最后一位、第一位。\textbf{如果我们称从开头开始的连续子串为前缀，从末尾开始的连续子串为后缀，那么在不依赖文本匹配的情况下，其实是判断已知部分的前后缀是否相等！}（找回了前世记忆）如果相等，那么在这个位置可以匹配，否则不需要考虑。而这就是KMP算法的主要思想所在，如果我们能预先处理出模板每个子串的最大前后缀长度，就能预先判断每次部分匹配时的最先跳转位置。让我们再看一组例子：
